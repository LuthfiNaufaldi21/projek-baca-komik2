...
  const [isSaving, setIsSaving] = useState(false); // Pastikan ini false saat awal
...
        if (bookmarked) {
            result = await removeBookmark(comic.id);
        } else {
            result = await addBookmark(comic.id);
        }

        if (!result.success) {
            setError(result.msg || "Gagal mengubah status bookmark.");
        }

    } catch (err) {
        setError("Terjadi kesalahan jaringan atau server.");
    } finally {
        // JAMINAN RESET: State ini HARUS disetel ke false, apa pun yang terjadi
        setIsSaving(false);
    }
  };
...
            </button>
            ...
            {error && (
                <p style={{ color: 'red', textAlign: 'center', marginTop: '-5px', marginBottom: '10px' }}>
                    {error}
                </p>
            )}
