...
function renderNavbar() {
  ...
  const navbarHTML = `
    <div class="flex justify-between items-center bg-white p-3 px-6 shadow-md">
      <a href="#home" class="inline-flex items-center" aria-label="Beranda">
        <img src="/logo.png" alt="KomikKita" class="h-8 sm:h-9 md:h-10 w-auto object-contain" />
      </a>
  ...
  return `
    <div class="bg-gray-100 p-4 rounded-lg mb-6">
      ...
      <div class="flex items-center gap-2 mb-4">
        <button id="clear-filter-btn" class="px-3 py-1.5 text-sm rounded-md bg-gray-200 text-gray-700 hover:bg-gray-300">Clear filter</button>
      </div>
...
function cleanupPageLogic() {
  ...
  // Jangan reset state filter/sort/page agar saat kembali dari detail ke daftar,
  // pilihan pengguna tetap tersimpan.
...
function applyFiltersAndRender() {
  ...
  let filteredComics = comics.filter((comic) => {
    ...
    // AND logic: semua tag terpilih harus ada di komik
    const tagsMatch =
  ...
  // Sinkronkan state tombol tag sesuai activeFilters
  const tagButtons = document.querySelectorAll('#filter-tag-container .filter-tag');
  tagButtons.forEach((btn) => {
    btn.classList.toggle('active', activeFilters.includes(btn.dataset.tag));
  });

  // Render chips ringkasan filter
  if (chipsContainer) {
    if (activeFilters.length === 0) {
      ...
      const chipsHTML = activeFilters
        .map((t) => `<span class="card-tag cursor-pointer" data-chip-tag="${t}" title="Klik untuk menghapus">${t} ✕</span>`)
        .join('');
      chipsContainer.innerHTML = `<div class="flex items-center gap-2 mb-4 flex-wrap"><span class="text-sm text-gray-600">Filter aktif:</span>${chipsHTML}</div>`;
...
function renderDaftarKomikPage() {
  ...
  appContent.innerHTML = `
    ...
    <div id="selected-filter-chips" class="hidden"></div>
    <div id="comic-grid-container"></div>
    <div id="pagination-container" class="mt-8"></div>
  `;
  ...
  document
    ...
    .addEventListener('click', (e) => {
      ...
      // Toggle multi-select (OR logic handled in applyFiltersAndRender)
  ...
  // Chips removal
  document
    .getElementById('selected-filter-chips')
    .addEventListener('click', (e) => {
      const chip = e.target.closest('[data-chip-tag]');
      if (!chip) return;
      const tag = chip.getAttribute('data-chip-tag');
      activeFilters = activeFilters.filter((t) => t !== tag);
      currentPage = 1;
      applyFiltersAndRender();
    });
