...
export default function Navbar() {
  ...
  // PERBAIKAN: Gunakan useRef untuk menyimpan posisi scroll terakhir
  // Ref tidak memicu render ulang saat nilainya berubah
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Logika: Kalau scroll ke bawah (current > last) DAN sudah lewat 100px -> Sembunyi
      if (currentScrollY > lastScrollY.current && currentScrollY > 100) {
        setIsVisible(false);
      } else {
        // Kalau scroll ke atas -> Muncul
        setIsVisible(true);
      }

      // Update nilai ref untuk putaran selanjutnya
      lastScrollY.current = currentScrollY;
    };

    // Pasang listener sekali saja saat komponen muncul
    window.addEventListener("scroll", handleScroll, { passive: true });

    return () => {
      window.removeEventListener("scroll", handleScroll);
    };
  }, []); // Dependency kosong = Listener dipasang sekali saja (Performansi Tinggi)
